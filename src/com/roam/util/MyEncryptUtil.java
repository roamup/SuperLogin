...
public class MyEncryptUtil {
    ...
    public static String encrypt(String xmlStr) {  
        ...
        byte[] md5Hasn = null;  
        try {  
            md5Hasn = MyEncryptUtil.MD5Hash(encrypt, 0, encrypt.length);  
        } catch (Exception e) {  
            e.printStackTrace();  
        }  
        // 组合消息体  
        byte[] totalByte = MyEncryptUtil.addMD5(md5Hasn, encrypt);  
  
        // 取密钥和偏转向量  
        byte[] key = new byte[8];  
        byte[] iv = new byte[8];  
        getKeyIV(MyEncryptUtil.key, key, iv);  
        ...
        byte[] temp = null;  
        try {  
            temp = MyEncryptUtil.DES_CBC_Encrypt(totalByte, deskey, ivParam);  
        } catch (Exception e) {  
            e.printStackTrace();  
        }  
    ...
    public static String decrypt(String xmlStr) throws Exception {  
        ...
        byte[] temp = null;  
        try {  
            temp = MyEncryptUtil.DES_CBC_Decrypt(encBuf, deskey, ivParam);  
        } catch (Exception e) {  
            e.printStackTrace();  
        }  
        ...
        byte[] md5Hash = null;  
        try {  
            md5Hash = MyEncryptUtil.MD5Hash(temp, 16, temp.length - 16);  
        } catch (Exception e) {  
            e.printStackTrace();  
        }  
